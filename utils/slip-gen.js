function convertToSlipFormat(jsonData, dateTimeStr, username) {
  ...
  function processBets(bets) {
    ...
    const betLines = bets
      .map((bet) => {
        ...
        const lottoTypes = Object.keys(bet.lotto_type)
          .filter((type) => bet.lotto_type[type])
          .join("");
        ...
        totalAmount += keys.reduce(
          (sum, key) => sum + (parseFloat(bet[key]) || 0),
          0
        );
        return `*${lottoTypes}\n${amounts.join(" ")}`;
      })
  ...
  const slipLines = [dateTimeStr, username, dateRangeStr, ...betLines, gtStr];
...
function convertToSlipFormatThai(jsonData, dateTimeStr, username) {
  ...
  function processBets(bets) {
    ...
    const betLines = bets
      .map((bet) => {
        ...
        keys.forEach((key) => {
          if (bet[key]) {
            amounts.push(`${bet[key]} (${reMapBetType[key]}), `);
          }
        });
        totalAmount += keys.reduce(
          (sum, key) => sum + (parseFloat(bet[key]) || 0),
  ...
  // Constructing the final slip
  const slipLines = [dateTimeStr, username, ...betLines, gtStr];
